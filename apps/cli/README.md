...
Command Line Interface for MeowCode - Run the MeowCode agent from the terminal without VSCode.

## Overview

This CLI uses the `@meow-code/vscode-shim` package to provide a VSCode API compatibility layer, allowing the main MeowCode extension to run in a Node.js environment.

## Installation

### Quick Install (Recommended)

Install the MeowCode CLI with a single command:

```bash
curl -fsSL https://raw.githubusercontent.com/MeowCodeInc/MeowCode/main/apps/cli/install.sh | sh
```
...
```bash
ROO_INSTALL_DIR=/opt/meow-code ROO_BIN_DIR=/usr/local/bin curl -fsSL ... | sh
```

**Install a specific version:**

```bash
ROO_VERSION=0.1.0 curl -fsSL https://raw.githubusercontent.com/MeowCodeInc/MeowCode/main/apps/cli/install.sh | sh
```
...
pnpm --filter meow-code bundle

# Build the CLI.
pnpm --filter @meow-code/cli build
```
...
meow "What is this project?" -w ~/Documents/my-project
```

You can also run without a prompt and enter it interactively in TUI mode:

```bash
meow -w ~/Documents/my-project
```
...
```bash
meow "Refactor the utils.ts file" --require-approval -w ~/Documents/my-project
```
...
```bash
# Prompt is required
meow --print "Summarize this repository"

# Create a new task with a specific session ID (UUID)
meow --print --create-with-session-id 018f7fc8-7c96-7f7c-98aa-2ec4ff7f6d87 "Summarize this repository"
```
...
```bash
printf '{"command":"start","requestId":"1","prompt":"1+1=?"}\n' | meow --print --stdin-prompt-stream --output-format stream-json
...
To use MeowCode Cloud features (like the provider proxy), you need to authenticate:

```bash
# Log in to MeowCode Cloud (opens browser)
meow auth login
...
1. Opens your browser to authenticate with MeowCode Cloud
2. Receives a secure token via localhost callback
3. Stores the token in `~/.config/meow/credentials.json`
...
| `--provider <provider>`                 | API provider (roo, anthropic, openai, openrouter, etc.)                                 | `openrouter` (or `meow` if authenticated) |
...
| Command           | Description                        |
| ----------------- | ---------------------------------- |
| `meow auth login`  | Authenticate with MeowCode Cloud   |
| `meow auth logout` | Clear stored authentication token  |
| `meow auth status` | Show current authentication status |
...
| ----------------- | -------------------------------------------------------------------- |
| `ROO_WEB_APP_URL` | Override the MeowCode Cloud URL (default: `https://app.TODOURL`) |
...
2. **ExtensionHost** (`extension-host.ts`):
    ...
    - Creates a VSCode API mock using `@meow-code/vscode-shim`
...
pnpm dev --provider meow --api-key $MEOW_API_KEY --print "Hello"
...
By default the `start` script points `MEOW_CODE_PROVIDER_URL` at `http://localhost:8080/proxy` for local development. To point at the production API instead, override the environment variable:

```bash
MEOW_CODE_PROVIDER_URL=https://api.TODOURL/proxy pnpm dev --provider meow --api-key $MEOW_API_KEY --print "Hello"
```
