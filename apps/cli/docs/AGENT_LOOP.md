...
This document explains how the MeowCode CLI detects and tracks the agent loop state.
...
const client = new ExtensionClient({
	...
	debug: true, // Writes to ~/.meow/cli-debug.log
})
...
The CLI uses type guards from `@meow-code/types` for categorization:

```typescript
import { isIdleAsk, isInteractiveAsk, isResumableAsk, isNonBlockingAsk } from "@meow-code/types"
...
Enable with `-d` flag. Logs go to `~/.meow/cli-debug.log`:

```bash
meow -d -P "Build something" --no-tui
```

View logs:

```bash
tail -f ~/.meow/cli-debug.log
```
