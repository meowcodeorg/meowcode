...
import { ExtensionMessage, MeowCodeMessage } from "@meow-code/types"
import { debugLog } from "@meow-code/core/cli"
...
export class MessageProcessor {
	 ...
	 * It contains the complete meowCodeMessages array which is the source of truth.
	 */
	private handleStateMessage(message: ExtensionMessage): void {
		...
		const { meowCodeMessages, mode } = message.state
		...
		if (!meowCodeMessages) {
			if (this.options.debug) {
				debugLog("[MessageProcessor] State message missing meowCodeMessages")
			}
			return
		}
		...
		this.store.setMessages(meowCodeMessages)
		...
		if (this.options.debug) {
			const lastMsg = meowCodeMessages[meowCodeMessages.length - 1]
			...
			debugLog("[MessageProcessor] State update", {
				messageCount: meowCodeMessages.length,
		...
		this.emitNewMessageEvents(previousState, currentState, meowCodeMessages)
	}
	...
	private handleMessageUpdated(message: ExtensionMessage): void {
		if (!message.meowCodeMessage) {
			if (this.options.debug) {
				debugLog("[MessageProcessor] messageUpdated missing meowCodeMessage")
			}
			return
		}
		...
		this.store.updateMessage(meowCodeMessage)

		const currentState = this.store.getAgentState()

		// Emit message updated event
		this.emitter.emit("messageUpdated", meowCodeMessage)
...
/**
 * Check if a message is a valid MeowCodeMessage.
 * Useful for validating messages before processing.
 */
export function isValidMeowCodeMessage(message: unknown): message is MeowCodeMessage {
 ...
 */
export function parseApiReqStartedText(message: MeowCodeMessage): { cost?: number } | undefined {
