...
import { type TokenUsage, type ToolUsage, MeowCodeEventName, taskEventSchema } from "@meow-code/types"
import type { Run, Task, TaskMetrics } from "@meow-code/evals"
...
export const useRunStatus = (run: Run): RunStatus => {
	...
	const onMessage = useCallback((messageEvent: MessageEvent) => {
		...
		switch (eventName) {
			case MeowCodeEventName.TaskStarted:
				startTimes.current.set(taskId, Date.now())
				break
			case MeowCodeEventName.TaskTokenUsageUpdated: {
			...
			case MeowCodeEventName.EvalPass:
			case MeowCodeEventName.EvalFail:
