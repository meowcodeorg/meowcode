...
describe("consolidateApiRequests", () => {
	...
	const createApiReqStarted = (ts: number, data: Record<string, unknown> = {}): MeowCodeMessage => ({
		ts,
		type: "say",
		say: "api_req_started",
	...
	const createApiReqFinished = (ts: number, data: Record<string, unknown> = {}): MeowCodeMessage => ({
		ts,
		type: "say",
	...
	const createTextMessage = (ts: number, text: string): MeowCodeMessage => ({
		ts,
		type: "say",
		say: "text",
		text,
	...
	it("should handle messages with no api_req pairs", () => {
		const messages: MeowCodeMessage[] = [createTextMessage(1000, "Hello"), createTextMessage(1001, "World")]
	...
	it("should handle single message array", () => {
		const messages: MeowCodeMessage[] = [createTextMessage(1000, "Hello")]
		const result = consolidateApiRequests(messages)
		expect(result).toEqual(messages)
	})
