...
describe("MockWorkspaceConfiguration", () => {
	...
	describe("Runtime Configuration", () => {
		...
		it("should return runtime config value over disk-based values", async () => {
			const config = new MockWorkspaceConfiguration("meow-code", context)
			...
			setRuntimeConfig("meow-code", "commandExecutionTimeout", 20)
		...
		it("should set and get runtime config values", () => {
			setRuntimeConfig("meow-code", "testSetting", "testValue")

			expect(getRuntimeConfig("meow-code.testSetting")).toBe("testValue")
		})

		it("should set multiple runtime config values at once", () => {
			setRuntimeConfigValues("meow-code", {
				setting1: "value1",
				setting2: 42,
				setting3: true,
			})

			expect(getRuntimeConfig("meow-code.setting1")).toBe("value1")
			expect(getRuntimeConfig("meow-code.setting2")).toBe(42)
			expect(getRuntimeConfig("meow-code.setting3")).toBe(true)
		})
		...
		it("should ignore undefined values in setRuntimeConfigValues", () => {
			...
			expect(getRuntimeConfig("meow-code.defined")).toBe("value")
			expect(getRuntimeConfig("meow-code.notDefined")).toBeUndefined()
		})

		it("should clear all runtime config values", () => {
			setRuntimeConfig("meow-code", "setting1", "value1")
			setRuntimeConfig("meow-code", "setting2", "value2")

			clearRuntimeConfig()

			expect(getRuntimeConfig("meow-code.setting1")).toBeUndefined()
			expect(getRuntimeConfig("meow-code.setting2")).toBeUndefined()
		})
