...
describe("getMeowModels", () => {
	const baseUrl = "https://api.TODOURL/proxy"
	...
	it("should fetch and parse models successfully", async () => {
		...
		const models = await getMeowModels(baseUrl, apiKey)

		expect(mockFetch).toHaveBeenCalledWith(
			"https://api.TODOURL/proxy/v1/models",
	...
	it("should work without API key", async () => {
		...
		const models = await getMeowModels(baseUrl)
	...
	it("should handle HTTP errors", async () => {
		...
		await expect(getMeowModels(baseUrl, apiKey)).rejects.toThrow(
			"Failed to fetch MeowCode Cloud models: HTTP 401: Unauthorized",
		)
	})
	...
	it("should handle timeout", async () => {
		...
		await expect(getMeowModels(baseUrl, apiKey)).rejects.toThrow(
			"Failed to fetch MeowCode Cloud models: Request timed out",
		)
	})
	...
	it("should handle invalid response format", async () => {
		...
		await expect(getMeowModels(baseUrl, apiKey)).rejects.toThrow(
			"Failed to fetch MeowCode Cloud models: Unexpected response format",
		)
	})
	...
	it("should normalize base URL correctly", async () => {
		...
		await getMeowModels("https://api.TODOURL/proxy/v1", apiKey)

		expect(mockFetch).toHaveBeenCalledWith("https://api.TODOURL/proxy/v1/models", expect.any(Object))
	})
	...
	it("should handle network errors", async () => {
		...
		await expect(getMeowModels(baseUrl, apiKey)).rejects.toThrow(
			"Failed to fetch MeowCode Cloud models: No response from server",
		)
	})
	...
	it("should apply highest versionedSettings for nightly builds (package name)", async () => {
		...
		;(Package as { name: string }).name = "meow-code-nightly"
