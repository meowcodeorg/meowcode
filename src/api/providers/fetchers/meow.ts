import { RooModelsResponseSchema, type ModelInfo, type ModelRecord } from "@meow-code/types"
...
/**
 * Fetches available models from the MeowCode Cloud provider
 *
 * @param baseUrl The base URL of the MeowCode Cloud provider
 * @param apiKey The API key (session token) for the MeowCode Cloud provider
 * @returns A promise that resolves to a record of model IDs to model info
 * @throws Will throw an error if the request fails or the response is not as expected.
 */
export async function getMeowModels(baseUrl: string, apiKey?: string): Promise<ModelRecord> {
	...
	try {
		...
		try {
			...
			if (!response.ok) {
				...
				console.error(`[getMeowModels] HTTP error:`, {
			...
			if (!parsed.success) {
				console.error("Error fetching MeowCode Cloud models: Unexpected response format", data)
				console.error("Validation errors:", parsed.error.format())
				throw new Error("Failed to fetch MeowCode Cloud models: Unexpected response format.")
			}
		...
		console.error("[getMeowModels] Error fetching MeowCode Cloud models:", {
		...
		if (error.name === "AbortError") {
			throw new Error("Failed to fetch MeowCode Cloud models: Request timed out after 10 seconds.")
		}

		// Handle fetch errors
		if (error.message?.includes("HTTP")) {
			throw new Error(`Failed to fetch MeowCode Cloud models: ${error.message}. Check base URL and API key.`)
		}

		// Handle network errors
		if (error instanceof TypeError) {
			throw new Error(
				"Failed to fetch MeowCode Cloud models: No response from server. Check MeowCode Cloud server status and base URL.",
			)
		}

		throw new Error(`Failed to fetch MeowCode Cloud models: ${error.message || "An unknown error occurred."}`)
	}
}
