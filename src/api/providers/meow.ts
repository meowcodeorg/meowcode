...
import { rooDefaultModelId, getApiProtocol, type ImageGenerationApiMethod } from "@meow-code/types"
...
export class MeowHandler extends BaseOpenAiCompatibleProvider<string> {
	...
	constructor(options: ApiHandlerOptions) {
		...
		let baseURL = process.env.MEOW_CODE_PROVIDER_URL ?? "https://api.TODOURL/proxy"
		...
		this.loadDynamicModels(this.fetcherBaseURL, sessionToken).catch((error) => {
			console.error("[MeowHandler] Failed to load dynamic models:", error)
		})
	}
	...
	override async *createMessage(
		...
		try {
			...
			const headers: Record<string, string> = {
				"X-Meow-App-Version": Package.version,
			}

			if (metadata?.taskId) {
				headers["X-Meow-Task-ID"] = metadata.taskId
			}
			...
			console.error(`[MeowHandler] Error during message streaming: ${JSON.stringify(errorContext)}`)
	...
	private async loadDynamicModels(baseURL: string, apiKey?: string): Promise<void> {
		try {
			...
			console.error("[MeowHandler] Error loading dynamic models:", {
				error: error instanceof Error ? error.message : String(error),
				stack: error instanceof Error ? error.stack : undefined,
				baseURL,
	...
	/**
	 * Generate an image using MeowCode Cloud's image generation API
