...
type MeowCodeSettingsKey = keyof MeowCodeSettings
...
export class ContextProxy {
	 ...
	 */
	private sanitizeProviderValues(values: MeowCodeSettings): MeowCodeSettings {
		...
		for (const key of legacyKeys) {
			if (key in sanitizedValues) {
				...
				sanitizedValues = copy as MeowCodeSettings
			}
		}
		...
		if (values.apiProvider !== undefined && !isKnownProvider) {
			...
			return restValues as MeowCodeSettings
		}
	...
	public async setValue<K extends MeowCodeSettingsKey>(key: K, value: MeowCodeSettings[K]) {
	...
	public getValue<K extends MeowCodeSettingsKey>(key: K): MeowCodeSettings[K] {
		return isSecretStateKey(key)
			? (this.getSecret(key as SecretStateKey) as MeowCodeSettings[K])
			: (this.getGlobalState(key as GlobalStateKey) as MeowCodeSettings[K])
	}

	public getValues(): MeowCodeSettings {
	...
	public async setValues(values: MeowCodeSettings) {
		const entries = Object.entries(values) as [MeowCodeSettingsKey, unknown][]
