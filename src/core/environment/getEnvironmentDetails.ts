...
import type { ExperimentId } from "@meow-code/types"
...
export async function getEnvironmentDetails(meowCode: Task, includeFileDetails: boolean = false) {
	let details = ""

	const meowCodeProvider = meowCode.providerRef.deref()
	const state = await meowCodeProvider?.getState()
	const { maxWorkspaceFiles = 200 } = state ?? {}

	// It could be useful for meowCode to know if the user went from one or no
	// file to another between messages, so we always include this context.
	const visibleFilePaths = vscode.window.visibleTextEditors
		?.map((editor) => editor.document?.uri?.fsPath)
		.filter(Boolean)
		.map((absolutePath) => path.relative(meowCode.cwd, absolutePath))
		.slice(0, maxWorkspaceFiles)

	// Filter paths through rooIgnoreController
	const allowedVisibleFiles = meowCode.rooIgnoreController
		? meowCode.rooIgnoreController.filterPaths(visibleFilePaths)
	...
	const openTabPaths = vscode.window.tabGroups.all
		...
		.filter(Boolean)
		.map((absolutePath) => path.relative(meowCode.cwd, absolutePath).toPosix())
		.slice(0, maxTabs)

	// Filter paths through rooIgnoreController
	const allowedOpenTabs = meowCode.rooIgnoreController
		? meowCode.rooIgnoreController.filterPaths(openTabPaths)
	...
	const busyTerminals = [
		...TerminalRegistry.getTerminals(true, meowCode.taskId),
		...TerminalRegistry.getBackgroundTerminals(true),
	]

	const inactiveTerminals = [
		...TerminalRegistry.getTerminals(false, meowCode.taskId),
		...TerminalRegistry.getBackgroundTerminals(false),
	]

	if (busyTerminals.length > 0) {
		if (meowCode.didEditFile) {
	...
	const recentlyModifiedFiles = meowCode.fileContextTracker.getAndClearRecentlyModifiedFiles()
	...
	if (maxGitStatusFiles > 0) {
		const gitStatus = await getGitStatus(meowCode.cwd, maxGitStatusFiles)
		if (gitStatus) {
			details += `\n\n# Git Status\n${gitStatus}`
		}
	}
	...
	if (includeCurrentCost) {
		const { totalCost } = getApiMetrics(meowCode.meowCodeMessages)
		details += `\n\n# Current Cost\n${totalCost !== null ? `$${totalCost.toFixed(2)}` : "(Not available)"}`
	}

	const { id: modelId } = meowCode.api.getModel()
	...
	if (includeFileDetails) {
		details += `\n\n# Current Workspace Directory (${meowCode.cwd.toPosix()}) Files\n`
		const isDesktop = arePathsEqual(meowCode.cwd, path.join(os.homedir(), "Desktop"))
		...
		if (isDesktop) {
			...
			if (maxFiles === 0) {
				...
				const [files, didHitLimit] = await listFiles(meowCode.cwd, true, maxFiles)
	...
	const todoListEnabled =
		state && typeof state.apiConfiguration?.todoListEnabled === "boolean"
			...
			: true
	const reminderSection = todoListEnabled ? formatReminderSection(meowCode.todoList) : ""
