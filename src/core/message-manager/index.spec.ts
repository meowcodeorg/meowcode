...
describe("MessageManager", () => {
	...
	describe("Basic rewind operations", () => {
		it("should remove messages at and after the target timestamp", async () => {
			...
			expect(mockTask.overwriteMeowCodeMessages).toHaveBeenCalledWith([
				{ ts: 100, say: "user", text: "First" },
				{ ts: 200, say: "assistant", text: "Response" },
			])
		...
		it("should keep target message when includeTargetMessage is true", async () => {
			...
			// Should keep messages up to and including ts=300 in meowCodeMessages
			expect(mockTask.overwriteMeowCodeMessages).toHaveBeenCalledWith([
				{ ts: 100, say: "user", text: "First" },
				{ ts: 200, say: "assistant", text: "Response" },
				{ ts: 300, say: "user", text: "Second" },
		...
		it("should throw error when timestamp not found", async () => {
			...
			await expect(manager.rewindToTimestamp(999)).rejects.toThrow(
				"Message with timestamp 999 not found in meowCodeMessages",
			)
		})
	...
	describe("Checkpoint scenarios", () => {
		it("should preserve Summary when checkpoint restore is BEFORE condense", async () => {
			...
			const meowCodeCall = mockTask.overwriteMeowCodeMessages.mock.calls[0][0]
			const hasCondenseContext = meowCodeCall.some((m: any) => m.say === "condense_context")
		...
		it("should preserve truncation marker when checkpoint restore is BEFORE truncation", async () => {
			...
			const meowCodeCall = mockTask.overwriteMeowCodeMessages.mock.calls[0][0]
			const hasTruncation = meowCodeCall.some((m: any) => m.say === "sliding_window_truncation")
	...
	describe("Combined scenarios", () => {
		...
		it("should handle empty meowCodeMessages array", async () => {
			mockTask.meowCodeMessages = []
			mockTask.apiConversationHistory = []
	...
	describe("Race condition handling", () => {
		it("should preserve assistant message when meowCodeMessage timestamp is earlier due to async execution", async () => {
			// This test reproduces the bug where deleting a user_feedback meowCodeMessage
			// incorrectly removes an assistant API message that was added AFTER the
			// meowCodeMessage (due to async tool execution during streaming).
			//
			// Timeline (race condition scenario):
			// - T1 (100): meowCodeMessage "user_feedback" created during tool execution
			// - T2 (200): assistant API message added when stream completes
			// - T3 (300): user API message (tool_result) added after pWaitFor
			//
			// When deleting the meowCodeMessage at T1, we should:
			...
			mockTask.apiConversationHistory = [
				...
				{
					ts: 200, // Race: added AFTER meowCodeMessage at ts=100
					role: "assistant",
			...
			// Delete the user_feedback meowCodeMessage at ts=100
			await manager.rewindToTimestamp(100)
		...
		it("should handle normal case where timestamps are properly ordered", async () => {
			// Normal case: meowCodeMessage timestamp aligns with API message timestamp
			mockTask.meowCodeMessages = [
				{ ts: 100, say: "user", text: "First" },
		...
		it("should handle multiple assistant messages before the user message in race condition", async () => {
			...
			mockTask.apiConversationHistory = [
				...
				{ ts: 150, role: "assistant", content: [{ type: "text", text: "First assistant msg" }] }, // After meowCodeMessage
				{ ts: 200, role: "assistant", content: [{ type: "text", text: "Second assistant msg" }] }, // After meowCodeMessage
