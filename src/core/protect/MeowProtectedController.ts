 ...
 */
export class MeowProtectedController {
	private cwd: string
	...
	constructor(cwd: string) {
		...
		this.ignoreInstance.add(MeowProtectedController.PROTECTED_PATTERNS)
	}
	...
	getProtectionMessage(): string {
		return "This is a MeowCode configuration file and requires approval for modifications"
	}
	...
	getInstructions(): string {
		const patterns = MeowProtectedController.PROTECTED_PATTERNS.join(", ")
		return `# Protected Files\n\n(The following MeowCode configuration file patterns are write-protected and always require approval for modifications, regardless of autoapproval settings. When using list_files, you'll notice a ${SHIELD_SYMBOL} next to files that are write-protected.)\n\nProtected patterns: ${patterns}`
	}
	...
	static getProtectedPatterns(): readonly string[] {
		return MeowProtectedController.PROTECTED_PATTERNS
	}
}
