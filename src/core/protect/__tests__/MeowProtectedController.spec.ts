...
import { MeowProtectedController } from "../MeowProtectedController"

describe("MeowProtectedController", () => {
	const TEST_CWD = "/test/workspace"
	let controller: MeowProtectedController

	beforeEach(() => {
		controller = new MeowProtectedController(TEST_CWD)
	})
	...
	describe("getProtectionMessage", () => {
		it("should return appropriate protection message", () => {
			...
			expect(message).toBe("This is a MeowCode configuration file and requires approval for modifications")
		})
	})
	...
	describe("getProtectedPatterns", () => {
		it("should return the list of protected patterns", () => {
			const patterns = MeowProtectedController.getProtectedPatterns()
