...
describe("Task persistence", () => {
	let mockProvider: MeowCodeProvider & Record<string, any>
	...
	beforeEach(() => {
		...
		) as MeowCodeProvider & Record<string, any>
	...
	// ── saveMeowCodeMessages ────────────────────────────────────────────────

	describe("saveMeowCodeMessages", () => {
		it("returns true on success", async () => {
			...
			const result = await (task as Record<string, any>).saveMeowCodeMessages()
			expect(result).toBe(true)
		})
		...
		it("snapshots the array before passing to saveTaskMessages", async () => {
			...
			await (task as Record<string, any>).saveMeowCodeMessages()

			expect(mockSaveTaskMessages).toHaveBeenCalledTimes(1)

			const callArgs = mockSaveTaskMessages.mock.calls[0][0]
			// The messages passed should be a COPY, not the live reference
			expect(callArgs.messages).not.toBe(task.meowCodeMessages)
			// But the content should be the same
			expect(callArgs.messages).toEqual(task.meowCodeMessages)
		})
	})
