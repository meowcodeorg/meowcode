...
import type { GlobalState, ProviderSettings, ModelInfo } from "@meow-code/types"
...
describe("MeowCode", () => {
	...
	describe("constructor", () => {
		it("should always have diff strategy defined", async () => {
			...
			expect(meowCode.diffStrategy).toBeDefined()
		})
		...
		it("should use default consecutiveMistakeLimit when not provided", () => {
			...
			expect(meowCode.consecutiveMistakeLimit).toBe(3)
		})
		...
		it("should respect provided consecutiveMistakeLimit", () => {
			...
			expect(meowCode.consecutiveMistakeLimit).toBe(5)
		})
		...
		it("should keep consecutiveMistakeLimit of 0 as 0 for unlimited", () => {
			...
			expect(meowCode.consecutiveMistakeLimit).toBe(0)
		})
		...
		it("should pass 0 to ToolRepetitionDetector for unlimited mode", () => {
			...
			expect(meowCode.toolRepetitionDetector).toBeDefined()
	...
	describe("getEnvironmentDetails", () => {
		describe("API conversation handling", () => {
			it.skip("should clean conversation history before sending to API", async () => {
				// MeowCode.create will now use our mocked getEnvironmentDetails
				const [meowCode, task] = Task.create({
					provider: mockProvider,
					apiConfiguration: mockApiConfig,
					task: "test task",
				})

				meowCode.abandoned = true
				await task
				...
				vi.spyOn(meowCode.api, "createMessage").mockImplementation(cleanMessageSpy)

				// Add test message to conversation history.
				meowCode.apiConversationHistory = [
					{
						role: "user" as const,
				...
				// Mock abort state
				Object.defineProperty(meowCode, "abort", {
					get: () => false,
					set: () => {},
					configurable: true,
				})
				...
				meowCode.apiConversationHistory = [messageWithExtra]

				// Trigger an API request
				await meowCode.recursivelyMakeMeowCodeRequests([{ type: "text", text: "test request" }], false)
			...
			it.skip("should handle image blocks based on model capabilities", async () => {
				...
				const [meowCodeWithImages, taskWithImages] = Task.create({
					provider: mockProvider,
				...
				vi.spyOn(meowCodeWithImages.api, "getModel").mockReturnValue({
					id: "claude-3-sonnet",
					info: {
						supportsImages: true,
				...
				meowCodeWithImages.apiConversationHistory = conversationHistory

				// Test with model that doesn't support images
				const [meowCodeWithoutImages, taskWithoutImages] = Task.create({
					provider: mockProvider,
				...
				vi.spyOn(meowCodeWithoutImages.api, "getModel").mockReturnValue({
					id: "gpt-3.5-turbo",
					info: {
				...
				meowCodeWithoutImages.apiConversationHistory = conversationHistory

				// Mock abort state for both instances
				Object.defineProperty(meowCodeWithImages, "abort", {
					get: () => false,
					set: () => {},
					configurable: true,
				})

				Object.defineProperty(meowCodeWithoutImages, "abort", {
					get: () => false,
					set: () => {},
					configurable: true,
				})
				...
				vi.spyOn(meowCodeWithImages.api, "createMessage").mockImplementation(imagesSpy)
				vi.spyOn(meowCodeWithoutImages.api, "createMessage").mockImplementation(noImagesSpy)

				// Set up conversation history with images
				meowCodeWithImages.apiConversationHistory = [
					{
						role: "user",
						content: [
				...
				meowCodeWithImages.abandoned = true
				await taskWithImages.catch(() => {})

				meowCodeWithoutImages.abandoned = true
				await taskWithoutImages.catch(() => {})

				// Trigger API requests
				await meowCodeWithImages.recursivelyMakeMeowCodeRequests([{ type: "text", text: "test request" }])
				await meowCodeWithoutImages.recursivelyMakeMeowCodeRequests([{ type: "text", text: "test request" }])
			...
			it.skip("should handle API retry with countdown", async () => {
				...
				const saySpy = vi.spyOn(meowCode, "say")
				...
				vi.spyOn(meowCode.api, "createMessage").mockImplementation(() => {
					if (firstAttempt) {
						firstAttempt = false
				...
				// Trigger API request
				const iterator = meowCode.attemptApiRequest(0)
				await iterator.next()
			...
			describe("processUserContentMentions", () => {
				it("should process mentions in user_message tags", async () => {
					...
					const { content: processedContent } = await processUserContentMentions({
						userContent,
						cwd: meowCode.cwd,
						fileContextTracker: meowCode.fileContextTracker,
					})
...
describe("Queued message processing after condense", () => {
	function createProvider(): any {
		...
		const provider = new MeowCodeProvider(ctx, output as any, "sidebar", new ContextProxy(ctx)) as any
