...
describe("Task.ask queued message drain", () => {
	it("consumes queued message while blocked on followup ask", async () => {
		...
		;(task as any).addToMeowCodeMessages = vi.fn(async () => {})
		;(task as any).saveMeowCodeMessages = vi.fn(async () => {})
		;(task as any).updateMeowCodeMessage = vi.fn(async () => {})
