...
describe("generateImageTool", () => {
	...
	describe("partial block handling", () => {
		it("should return early when block is partial", async () => {
			...
			await generateImageTool.handle(mockMeowCode as Task, partialBlock as ToolUse<"generate_image">, {
			...
			expect(mockMeowCode.say).not.toHaveBeenCalled()
		})
		...
		it("should process when block is not partial", async () => {
			...
			await generateImageTool.handle(mockMeowCode as Task, completeBlock as ToolUse<"generate_image">, {
				askApproval: mockAskApproval,
				handleError: mockHandleError,
				pushToolResult: mockPushToolResult,
			})
		...
		it("should add cache-busting parameter to image URI", async () => {
			...
			mockMeowCode.providerRef.deref().convertToWebviewUri = vi.fn().mockReturnValue(mockWebviewUri)
			...
			// Check that meowCode.say was called with image data containing cache-busting parameter
			expect(mockMeowCode.say).toHaveBeenCalledWith("image", expect.stringMatching(/"imageUri":"[^"]+\?t=\d+"/))

			// Verify the imageUri contains the cache-busting parameter
			const sayCall = mockMeowCode.say.mock.calls.find((call: any[]) => call[0] === "image")
			if (sayCall) {
	...
	describe("missing parameters", () => {
		it("should handle missing prompt parameter", async () => {
			...
			await generateImageTool.handle(mockMeowCode as Task, block as ToolUse<"generate_image">, {
				askApproval: mockAskApproval,
				handleError: mockHandleError,
				pushToolResult: mockPushToolResult,
			})
			...
			expect(mockMeowCode.recordToolError).toHaveBeenCalledWith("generate_image")
			expect(mockMeowCode.sayAndCreateMissingParamError).toHaveBeenCalledWith("generate_image", "prompt")
		...
		it("should handle missing path parameter", async () => {
			...
			expect(mockMeowCode.consecutiveMistakeCount).toBe(1)
			expect(mockMeowCode.recordToolError).toHaveBeenCalledWith("generate_image")
			expect(mockMeowCode.sayAndCreateMissingParamError).toHaveBeenCalledWith("generate_image", "path")
			expect(mockPushToolResult).toHaveBeenCalledWith("Missing parameter error")
		})
	})
	...
	describe("experiment validation", () => {
		it("should error when image generation experiment is disabled", async () => {
			...
			mockMeowCode.providerRef.deref().getState.mockResolvedValue({
				experiments: {
	...
	describe("input image validation", () => {
		it("should handle non-existent input image", async () => {
			...
			expect(mockMeowCode.say).toHaveBeenCalledWith("error", expect.stringContaining("Input image not found"))
		...
		it("should handle unsupported image format", async () => {
			...
			expect(mockMeowCode.say).toHaveBeenCalledWith("error", expect.stringContaining("Unsupported image format"))
