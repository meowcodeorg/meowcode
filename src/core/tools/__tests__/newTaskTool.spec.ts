...
describe("newTaskTool", () => {
	...
	describe("VSCode setting: newTaskRequireTodos", () => {
		...
		it("should check VSCode setting with Package.name configuration key", async () => {
			...
			expect(mockGetConfiguration).toHaveBeenCalledWith("meow-code")
			expect(mockGet).toHaveBeenCalledWith("newTaskRequireTodos", false)
		})

		it("should use current Package.name value (meow-code-nightly) when accessing VSCode configuration", async () => {
			...
			;(pkg.Package as any).name = "meow-code-nightly"
			...
			expect(mockGetConfiguration).toHaveBeenCalledWith("meow-code-nightly")
			expect(mockGet).toHaveBeenCalledWith("newTaskRequireTodos", false)
		})
	})
