...
describe("webviewMessageHandler delete functionality", () => {
	...
	beforeEach(() => {
		...
		getCurrentTaskMock = {
			...
			overwriteMeowCodeMessages: vi.fn(async () => {}),
	...
	describe("handleDeleteMessageConfirm", () => {
		it("should handle deletion when apiConversationHistoryIndex is -1 (message not in API history)", async () => {
			...
			getCurrentTaskMock.meowCodeMessages = [
			...
			// Verify that meowCodeMessages was truncated at the correct index
			expect(getCurrentTaskMock.overwriteMeowCodeMessages).toHaveBeenCalledWith([])
		...
		it("should handle deletion when message not found in meowCodeMessages", async () => {
			getCurrentTaskMock.meowCodeMessages = [{ ts: 1000, say: "user", text: "Some message" }]
			...
			expect(getCurrentTaskMock.overwriteMeowCodeMessages).not.toHaveBeenCalled()
		...
		it("should handle deletion with attempt_completion in API history", async () => {
			...
			// Verify that meowCodeMessages was truncated
			expect(getCurrentTaskMock.overwriteMeowCodeMessages).toHaveBeenCalledWith([])
		...
		describe("condense preservation behavior", () => {
			...
			it("should remove Summary when its condense_context meowCodeMessage is deleted", async () => {
