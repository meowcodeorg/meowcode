...
describe("webviewMessageHandler - Edit Message with Timestamp Fallback", () => {
	...
	beforeEach(() => {
		...
		mockCurrentTask = {
			taskId: "test-task-id",
			meowCodeMessages: [] as MeowCodeMessage[],
	...
	it("should not modify API history when apiConversationHistoryIndex is -1", async () => {
		...
		// UI messages (meowCodeMessages)
		mockCurrentTask.meowCodeMessages = [
			{
				ts: userMessageTs,
				type: "say",
				say: "user_feedback",
		...
		expect(mockCurrentTask.overwriteMeowCodeMessages).toHaveBeenCalledWith(
