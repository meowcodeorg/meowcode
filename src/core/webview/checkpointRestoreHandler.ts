...
export async function handleCheckpointRestoreOperation(config: CheckpointRestoreConfig): Promise<void> {
	const { provider, currentMeowCode, messageTs, checkpoint, operation, editData } = config
	...
	try {
		...
		if (operation === "delete" && currentMeowCode && !currentMeowCode.abort) {
			currentMeowCode.abortTask()
			// Wait a bit for the abort to complete
			await pWaitFor(() => currentMeowCode.abort === true, {
				timeout: 1000,
				interval: 50,
			}).catch(() => {
		...
		if (operation === "edit" && editData) {
			const operationId = `task-${currentMeowCode.taskId}`
		...
		await currentMeowCode.checkpointRestore({
			ts: messageTs,
		...
		if (operation === "delete") {
			...
			const { historyItem } = await provider.getTaskWithId(currentMeowCode.taskId)
 ...
 */
export async function waitForMeowCodeInitialization(provider: MeowCodeProvider, timeoutMs: number = 3000): Promise<boolean> {
	try {
