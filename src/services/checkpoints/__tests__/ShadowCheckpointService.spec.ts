	...
	(klass, prefix) => {
		...
		describe(`${klass.name}#hasNestedGitRepositories`, () => {
			it("throws error when nested git repositories are detected during initialization", async () => {
				...
				await mainGit.init()
				await mainGit.addConfig("user.name", "MeowCode")
				await mainGit.addConfig("user.email", "support@TODOURL")
				...
				await nestedGit.addConfig("user.name", "MeowCode")
				await nestedGit.addConfig("user.email", "support@TODOURL")
