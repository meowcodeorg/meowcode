 ...
 *
 * Provides proxy configuration for all outbound HTTP/HTTPS requests from the MeowCode extension.
...
async function configureUndiciProxy(config: ProxyConfig): Promise<void> {
	...
	try {
		...
		// To ensure MeowCode's `fetch()` calls are proxied, patch global fetch in debug mode.
