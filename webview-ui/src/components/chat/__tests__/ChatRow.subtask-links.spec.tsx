...
import type { HistoryItem, MeowCodeMessage } from "@meow-code/types"
	...
	useTranslation: () => ({
		t: (key: string) => {
			const map: Record<string, string> = {
				"chat:subtasks.wantsToCreate": "Meow wants to create a new subtask",
...
let mockMeowCodeMessages: MeowCodeMessage[] = []
...
function renderChatRow(message: any, currentTaskItem?: Partial<HistoryItem>, meowCodeMessages?: MeowCodeMessage[]) {
	mockCurrentTaskItem = currentTaskItem
	mockMeowCodeMessages = meowCodeMessages || [message]
...
describe("ChatRow - subtask links", () => {
	...
	describe("newTask tool", () => {
		...
		it("should not display 'Go to subtask' link when directly followed by subtask_result", () => {
			...
			// Pass both messages in the meowCodeMessages array
