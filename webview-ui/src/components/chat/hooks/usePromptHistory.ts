import { MeowCodeMessage, HistoryItem } from "@meow-code/types"
...
export const usePromptHistory = ({
	...
	const filteredPromptHistory = useMemo(() => {
		// First try to get conversation messages (user_feedback from meowCodeMessages)
		const conversationPrompts = meowCodeMessages
		...
		// If we have meowCodeMessages array (meaning we're in an active task), don't fall back to task history
		// Only use task history when starting fresh (no active conversation)
		if (meowCodeMessages?.length) {
			return []
		}
	...
	}, [meowCodeMessages, taskHistory, cwd])
